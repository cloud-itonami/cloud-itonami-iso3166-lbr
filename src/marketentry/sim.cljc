...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (LBR, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up foreign-ownership-ineligible) ==")
    ...
    (println "== filing/submit eng-4 (foreign-ownership-ineligible -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up tin-record-unverified) ==")
    ...
    (println "== filing/submit eng-5 (tin-record-unverified -> HARD hold) ==")
