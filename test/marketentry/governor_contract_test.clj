  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Monrovia Fabrication Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Monrovia Fabrication Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest foreign-ownership-ineligible-is-held-and-unoverridable
  (testing "declared :foreign-company? true for a Schedule-Two sector but declared capital fails its own ownership-tier's Investment Act of 2010 threshold -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:foreign-ownership-ineligible} (-> (store/ledger db) last :basis)))
...
(deftest tin-record-unverified-is-held-and-unoverridable
  (testing "unverified LRA TIN record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:tin-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
